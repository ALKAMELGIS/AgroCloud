...
POST /predict returns axis-aligned boxes (always) and optional instance mask
polygons in image-pixel coordinates when a segmentation engine is active.

Engines (TREE_DETECTION_ENGINE or auto):
  • deepforest  — zero-config boxes (default)
  • yolo / yolo_seg — Ultralytics .pt (seg extracts masks when available)
  • onnx_seg — ONNX Runtime when MODEL_PATH ends with .onnx
"""
...
# Only used when explicitly allowed — COCO general weights are NOT tree detectors.
YOLO_FALLBACK_WEIGHTS = os.environ.get("TREE_DETECTION_YOLO_WEIGHTS", "yolo11n-seg.pt").strip()
...
def _select_engine() -> str:
    """Prefer DeepForest (tree-specific). YOLO only when real tree weights exist."""
    if ENGINE in ("deepforest", "yolo", "yolo_seg", "onnx_seg"):
        if ENGINE in ("yolo", "yolo_seg") and not _find_tree_yolo_weights() and not ALLOW_COCO_YOLO:
            return "deepforest"
    ...
    if MODEL_PATH and os.path.exists(MODEL_PATH):
        if MODEL_PATH.lower().endswith(".onnx"):
            return "onnx_seg"
        if not _is_coco_general_weights(MODEL_PATH):
            try:
...
def _resolve_yolo_weights(weights: str | None = None) -> str:
    ...
    raise RuntimeError(
        "No tree-trained YOLO weights found (set MODEL_PATH to a tree .pt, or use "
        "TREE_DETECTION_ENGINE=deepforest). COCO yolo11n-seg.pt is disabled for crowns."
    )
...
def _build_engine() -> tuple[Any, str | None]:
    """Prefer DeepForest first — never silently fall through to COCO YOLO."""
    preferred = _select_engine()
    order = [preferred]
    for k in ("deepforest", "yolo_seg", "onnx_seg"):
        if k not in order:
            order.append(k)
    # Drop YOLO from fallback chain unless tree weights (or explicit COCO) exist.
    if not _find_tree_yolo_weights() and not ALLOW_COCO_YOLO:
        order = [k for k in order if k not in ("yolo", "yolo_seg")]
        if "deepforest" not in order:
            order.insert(0, "deepforest")
    errors: list[str] = []
    for kind in order:
        try:
...
def _resolve_named_engine(kind: str | None):
    ...
    if name == "yolo":
        name = "yolo_seg"
    if name not in ("deepforest", "yolo_seg", "onnx_seg"):
        return None
    if name in _named_engines:
        return _named_engines[name]
    if name in ("yolo_seg", "yolo") and not _find_tree_yolo_weights() and not ALLOW_COCO_YOLO:
        # Fall back to DeepForest rather than COCO — better crowns, correct places.
        name = "deepforest"
        if name in _named_engines:
            return _named_engines[name]
    ...
    if not cid:
        ...
        if _engine is None:
            raise RuntimeError(
                ...
                "Install deepforest (recommended) in the service venv: "
...
async def predict(
    ...
    if "instances" not in result:
        result["instances"] = [_box_to_instance(b) for b in result.get("boxes") or []]
    if "engine" not in result:
        result["engine"] = getattr(eng, "name", "unknown")
