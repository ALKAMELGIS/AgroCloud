...
export function TrainingAITool({
  ...
  useEffect(() => {
    ...
    const apply = (h: Awaited<ReturnType<typeof fetchTrainingHealth>>) => {
      ...
      if (!h.training) {
        setHealthNote(h.error || 'Training service not ready on :8095 â start segformer-detection.')
        return
      }
      setHealthNote(null)
      // Clear stale transport errors once :8095 is healthy again.
      setTrainError(prev =>
        prev &&
        /fetch failed|offline|Could not reach|transformers|ModuleNotFound|Failed to fetch|service offline/i.test(
          prev,
        )
          ? null
          : prev,
    ...
    const probe = () => {
      void fetchTrainingHealth(ac.signal)
        ...
        .catch(err => {
          ...
          setHealthNote('SegFormer / Training service offline (port 8095).')
        })
    }
    probe()
  ...
  const handleTrain = useCallback(async () => {
    ...
    if (!selected?.trainableOnAgroCloud || !selected.trainEncoder) {
      setTrainError('Select a model that can train on AgroCloud (:8095).')
      return
    }
      ...
      if (done.status === 'error') {
        ...
        setTrainError(
          /ModuleNotFoundError:\s*No module named 'transformers'/i.test(raw)
            ? 'Training deps missing on :8095 (transformers). Restart segformer-detection with its .venv.'
            : /ECONNREFUSED/i.test(raw)
              ? 'Training service offline (port 8095). Start segformer-detection and retry.'
              : raw,
        )
