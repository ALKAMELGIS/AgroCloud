...
export type UseTreeDetectionState = {
  ...
  error: string | null
  busy: boolean
  /** Always false — local crown CV fallback was removed; ML model is required. */
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // DeepForest scores are often low (0.1–0.4); keep a permissive band.
        const scoreThreshold = Math.max(0.05, Math.min(0.45, 0.32 - effectiveSensitivity * 0.25))

        const detectBoxes = async (canvas: HTMLCanvasElement) =>
          predictTreeBoxes(canvas, {
        ...
        for (let attempt = 0; attempt < 4; attempt += 1) {
          ...
          for (const chunk of chunks) {
            ...
            const boxes = await detectBoxes(mosaic.canvas)
            if (isDone()) return
            passes.push(
              crownsFromBoxes({
                boxes,
        ...
        setState({
          ...
          result: null,
          error: offline
            ? 'Tree Detection Model is offline. Start backend/services/tree-detection (DeepForest) then Run again.'
