...
export function GisSelectionResultsPanel({
  ...
  const isolation = useMapOverlayIsolation(open)
  if (!open) return null

  return (
    <div className="gis-sel-results-shell" role="presentation">
      <button
        type="button"
        className="gis-sel-results-backdrop"
        aria-label="Close selection results"
        onClick={onClose}
      />
      <aside
        {...isolation}
        className="gis-sel-results gis-sel-results--modal"
        role="dialog"
        aria-modal="true"
