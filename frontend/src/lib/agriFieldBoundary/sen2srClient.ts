...
export type Sen2srProductMode = 'raw' | 'sen2sr'
...
function offlineLike(raw: string | null | undefined): boolean {
  return /offline|ECONNREFUSED|Could not reach|fetch failed|network|timed out|TimeoutError|:8092|uvicorn app:app/i.test(
    String(raw || ''),
  )
}
