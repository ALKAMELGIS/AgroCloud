...
export type FieldBoundaryHealth = {
  ...
  ftw_infer?: boolean
  ftw_live?: boolean
  /** SEN2SRLite neural SR available on the same :8092 service. */
  sen2sr?: boolean
  sen2sr_error?: string
  offline?: boolean
}
...
export async function fetchFieldBoundaryHealth(signal?: AbortSignal): Promise<FieldBoundaryHealth> {
  ...
  try {
    ...
    if (json && typeof json === 'object' && String(json.status || '') === 'ok') {
      return { ...json, offline: false }
    }
    return { offline: true, status: 'offline' }
  } catch (err) {
    // Let callers skip apply on unmount; do not treat abort as "service offline".
