...
/** Pivot circles must stay whole — they are never carved, neighbours yield instead. */
function isPivotCircle(f: GeoJSON.Feature): boolean {
  return String((f.properties as Record<string, unknown> | null)?.footprint_method || '') === 'pivot-circle'
}

/** Keep the biggest ring after a carve — slivers read as stray strokes. */
function largestPolygonPart(f: GeoJSON.Feature): GeoJSON.Feature | null {
  const g = f.geometry
  if (!g) return null
  ...
  return best ? { ...f, geometry: { type: 'Polygon', coordinates: best } } : null
}

/** Cut already-kept parcels out of `f` so no two field polygons cover the same ground. */
function carveAgainstKept(f: GeoJSON.Feature, kept: GeoJSON.Feature[]): GeoJSON.Feature | null {
  let current: GeoJSON.Feature | null = f
  for (const winner of kept) {
    if (!current) return null
    try {
      if (!turf.booleanIntersects(current as any, winner as any)) continue
      const diff = turf.difference(turf.featureCollection([current as any, winner as any]))
      if (!diff?.geometry) return null
      current = largestPolygonPart({ ...current, geometry: diff.geometry })
    } catch {
      return current
    }
  }
...
/**
 * Resolve overlapping field polygons: drop stacked duplicates outright, then
 * carve whatever still overlaps a higher-priority parcel so the layer is strictly
 * disjoint. Pivot circles rank first and are never carved.
 */
export function resolveFieldPolygonOverlaps(
  ...
  const ranked = sanitized.features
    .map(f => ({
      ...
      pivot: isPivotCircle(f),
    }))
    .sort((a, b) => Number(b.pivot) - Number(a.pivot) || b.conf - a.conf || b.area - a.area)

  const kept: GeoJSON.Feature[] = []

  for (const { f, area, pivot } of ranked) {
    let overlaps = false
    for (const winner of kept) {
      try {
    ...
    const carved = carveAgainstKept(f, kept)
    if (!carved?.geometry) continue
    const carvedArea = featureAreaM2(carved)
    if (!(carvedArea >= minArea) || carvedArea / Math.max(area, 1) < 0.2) continue
    kept.push(
      carvedArea < area * 0.999
        ? {
            ...carved,
            properties: {
              ...(carved.properties || {}),
              overlap_carved: true,
              area_m2: Math.round(carvedArea * 10) / 10,
            },
          }
        : carved,
    )
