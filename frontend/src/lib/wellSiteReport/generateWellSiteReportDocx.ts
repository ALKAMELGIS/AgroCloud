...
import { buildWellSiteReportPayload } from './buildWellSiteReportPayload'
import { buildWellSiteDocxDocumentXml } from './buildWellSiteDocxDocument'
import { base64ToUint8, buildWellSiteDocxModel } from './wellSiteReportDocxModel'
import {
  buildDocxChartXml,
...
export async function generateWellSiteReportDocx(
  ...
  if (coreFile) {
    ...
    coreXml = coreXml.replace(
      /<dc:title>.*?<\/dc:title>/,
      `<dc:title>Well Site Recommendation Report (Hydro-AI)</dc:title>`,
    )
  ...
  a.href = url
  a.download = `Well_Site_Recommendation_Report_${safeName || 'AOI'}.docx`
  a.click()
