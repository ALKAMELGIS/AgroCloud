...
function userFacingError(json: Record<string, unknown>, fallback: string): string {
  ...
  if (isOfflineDetail(detail) || (/offline|ECONNREFUSED/i.test(raw) && !transientLike(raw))) {
    if (/inference/i.test(fallback)) {
      return 'Inference is unavailable. Confirm the AgroCloud API is running.'
    }
    return 'Training is unavailable. Confirm the AgroCloud API is running.'
  }
...
export async function fetchTrainingHealth(signal?: AbortSignal): Promise<{
  ...
  try {
    // Prefer dedicated /api/training/health so Detect's spectral-builtin "ok" is not treated as TRAIN-ready.
    ...
    if ((res.ok || res.status === 503) && looksLikeHealth) {
      const training =
        Boolean(json.training) ||
        String(json.status || '') === 'busy' ||
        (Boolean(json.builtin_fallback) && String(json.status || '') === 'ok')
      return {
    ...
    return {
      ...
      error: training
        ...
        : json2.training_error != null
          ...
          : 'Training service is not ready on the AgroCloud API.',
    }
  } catch (err) {
    ...
    return {
      ...
      training: false,
      onnx: false,
      error: 'Training is unavailable. Confirm the AgroCloud API is running.',
    }
  }
}
...
/** Checkpoints via `/api/training/models` (newest first). */
...
/**
 * Longest saved epoch curve across all SegFormer checkpoints on the training API.
