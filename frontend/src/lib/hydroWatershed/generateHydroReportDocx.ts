...
import { buildHydroReportPayload } from './buildHydroReportPayload'
import { buildHydroDocxDocumentXml } from './buildHydroDocxDocument'
...
export async function generateHydroWatershedReportDocx(input: BuildHydroReportInput): Promise<void> {
  ...
  if (coreFile) {
    ...
    coreXml = coreXml.replace(
      /<dc:title>.*?<\/dc:title>/,
      `<dc:title>Hydro Watershed &amp; Flood Risk Assessment Report</dc:title>`,
    )
  ...
  a.href = url
  a.download = `Hydro_Watershed_Report_${safeName}.docx`
  a.click()
