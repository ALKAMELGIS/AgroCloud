...
export async function generateEoEnrichmentReportDocx(input: {
  ...
  if (coreFile) {
    ...
    coreXml = coreXml.replace(
      /<dc:title>.*?<\/dc:title>/,
      `<dc:title>EO Layer Enrichment Report</dc:title>`,
    )
    zip.file('docProps/core.xml', coreXml)
  }

  const safeName = model.layerName.replace(/[^\w.-]+/g, '_').slice(0, 40) || 'layer'
  ...
  a.href = url
  a.download = `EO_Enrichment_Report_${safeName}.docx`
  a.click()
